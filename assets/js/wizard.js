...
(function ($) {
  ...
  var VortemWizard = {
    ...
    formatLoadingText: function (text) {
      ...
      var isRTL =
        $("html").attr("dir") === "rtl" ||
        $("body").hasClass("rtl") ||
        $(".vortem-wizard-card").hasClass("vortem-wizard-rtl");
    ...
    bindEvents: function () {
      ...
      $(document).on(
        "click",
        "#restart-wizard-confirm",
        this.confirmRestartWizard,
      );
      $(document).on(
        "click",
        "#restart-wizard-cancel",
        this.cancelRestartWizard,
      );
      $(document).on(
        "click",
        ".vortem-wizard-modal-overlay",
        this.cancelRestartWizard,
      );
      ...
      $(document).on(
        "click",
        "#wizard-currency-select-display",
      ...
      $(document).on("click", function (e) {
        ...
        if (
          !$currencySelect.is(e.target) &&
          $currencySelect.has(e.target).length === 0
        ) {
    ...
    initHistory: function () {
      ...
      window.history.replaceState(
        { vortemWizard: true, step: this.getCurrentStep() },
        "",
        window.location.href,
      );
      $(window).on("popstate.vortemWizard", function () {
        if (!$(".vortem-wizard-card").length) {
          return;
        }
        VortemWizard.navigateToStep(window.location.href, {
          pushState: false,
        });
      });
    },

    getStepFromUrl: function (url) {
      try {
        var parsed = new URL(url, window.location.origin);
        return parseInt(parsed.searchParams.get("step"), 10) || 1;
      } catch (e) {
        return 1;
      }
    },
    ...
    /**
     * Swap wizard rail + pane in place (no full admin reload).
     * Falls back to a full navigation when the fragment cannot be loaded.
     */
    navigateToStep: function (url, options) {
      options = options || {};
      var self = this;

      if (this._stepNavInFlight) {
        return;
      }
      this._stepNavInFlight = true;

      var $card = $(".vortem-wizard-card").first();

      if (!$card.length) {
        this._stepNavInFlight = false;
        window.location.href = url;
        return;
      }

      this.closeCurrencyDropdown();
      $("#restart-wizard-modal").hide();

      var $pane = $card.find(".wizard-pane").first();
      var paneHeight = $pane.length ? $pane.outerHeight() : 0;
      var $container = $(".vortem-wizard-container").first();
      var containerTop = $container.length ? $container.offset().top : 0;

      if (paneHeight) {
        $pane.css("min-height", paneHeight + "px");
      }
      $("body").addClass("vortem-wizard-navigating");
      ...
      $.ajax({
        ...
        success: function (html) {
          var $parsed = $("<div>").append(
            $.parseHTML(html, document, false),
          );
          var $newCard = $parsed.find(".vortem-wizard-card").first();

          if (!$newCard.length) {
            self._stepNavInFlight = false;
            window.location.href = url;
            return;
          }

          $card.find(".wizard-rail").replaceWith($newCard.find(".wizard-rail"));
          $card.find(".wizard-pane").replaceWith($newCard.find(".wizard-pane"));

          $("body").addClass("vortem-wizard-same-session");
          $("body").removeClass("vortem-wizard-navigating");
          $card.find(".wizard-pane").css("min-height", "");

          if ($container.length) {
            var delta = $container.offset().top - containerTop;
            if (delta) {
          ...
          if (options.pushState !== false && window.history.pushState) {
            window.history.pushState(
              { vortemWizard: true, step: self.getStepFromUrl(url) },
              "",
              url,
            );
          }
    ...
    showCurrencyBoxLoading: function () {
      ...
      var raw =
        vortemWizard.strings && vortemWizard.strings.loading
          ? vortemWizard.strings.loading
          : "Loading";
      var loadingText = raw.replace(/\.+$/, "");
      var dotsHtml =
        '<span class="vortem-currency-loading-dots"><span>.</span><span>.</span><span>.</span></span>';
      $display.addClass("loading initialized");
      $display
        .find(".vortem-currency-text")
        .html(
          '<div class="currency-line-1">' +
            loadingText +
            dotsHtml +
            '</div><div class="currency-line-2"></div>',
        );
    },
    ...
    preInitCurrencyDisplay: function () {
      ...
      if ($currencyDisplay.length) {
        ...
        if (!$text.find(".currency-line-1").length) {
          $text.html(
            '<div class="currency-line-1"></div><div class="currency-line-2"></div>',
          );
        }
    ...
    loadCurrencies: function () {
      ...
      var $display = $("#wizard-currency-select-display");
      var currentCurrency =
        $currencySelect.data("current") || $currencySelect.val() || "USD";
      ...
      $.ajax({
        ...
        success: function (response) {
          ...
          if (response.success && response.data) {
            ...
            var $dropdownList = $dropdown.find(
              ".vortem-currency-dropdown-list",
            );
            ...
            var $searchBox = $(
              '<div class="vortem-currency-search-wrapper">' +
            ...
            if (Array.isArray(currencies)) {
              currencies.forEach(function (currency) {
                var code =
                  currency.currency_code || currency.code || currency.id;
                var countryCurrency =
                ...
                if (svgContent && typeof svgContent === "string") {
                  ...
                  if (
                    normalizedSvg &&
                    normalizedSvg.trim().match(/<svg[\s>]/i)
                  ) {
                    // Use SVG for display
                    flag = normalizedSvg;
                    flagType = "svg";
            ...
            for (
              ...
              if (
                groupedCurrencies[letter] &&
                groupedCurrencies[letter].length > 0
              ) {
                var $group = $("<div></div>")
                  .addClass("vortem-currency-group")
                  .attr("data-letter", letter);
                var $groupHeader = $("<div></div>")
                  .addClass("vortem-currency-group-header")
                  .text(letter);
                ...
                groupedCurrencies[letter].forEach(function (currency) {
                  ...
                  if (currency.flagType === "svg" && currency.flag) {
                    ...
                    flagHtml =
                      '<div class="vortem-currency-flag-container">' +
                      '<div class="vortem-currency-flag vortem-currency-flag-svg">' +
                      currency.flag +
                      "</div>" +
                      "</div>";
                  } else if (currency.flag) {
                    flagHtml =
                      '<span class="vortem-currency-flag">' +
                      currency.flag +
                      "</span>";
                  }
                  ...
                  var currencyName = currency.name
                    ? currency.name +
                      ' (<span class="vortem-currency-code">' +
                      currency.code +
                      "</span>)"
                    : '<span class="vortem-currency-code">' +
                      currency.code +
                  ...
                  $option.html(
                    flagHtml +
                      '<span class="vortem-currency-name">' +
                      currencyName +
                      "</span>" +
                      (isSelected
            ...
            var $searchInput = $dropdownList.find(
              ".vortem-currency-search-input",
            );
              ...
              if (searchTerm === "") {
                ...
                $dropdownList.find(".vortem-currency-group").each(function () {
                  ...
                  $options.each(function () {
                    ...
                    if (
                      currencyName.indexOf(searchTerm) !== -1 ||
                      currencyCode.indexOf(searchTerm) !== -1
                    ) {
                      $option.show();
                      hasMatch = true;
                ...
                var $noResults = $dropdownList.find(
                  ".vortem-currency-no-results",
                );
                if (!hasVisibleItems) {
                  if ($noResults.length === 0) {
                    $dropdownList.append(
                      '<div class="vortem-currency-no-results">' +
                        (vortemWizard.strings.no_results ||
                          "No currencies found") +
                        "</div>",
                    );
                  } else {
            ...
            if (selectedCurrency) {
              ...
              var shouldSwitch =
                !currentCurrency || currentCurrency !== selectedCurrency.code;
              if (shouldSwitch) {
              ...
              $text
                .find(".currency-line-1")
                .text(vortemWizard.strings.selectCurrency || "Select Currency");
              $text.find(".currency-line-2").text("");
    ...
    updateCurrencyDisplay: function (code, name, flag, flagType, svgContent) {
      ...
      if (flagType === "svg" && svgContent) {
        ...
        $flagPlaceholder.html(
          '<span class="vortem-currency-flag">' + flag + "</span>",
        );
      }
    ...
    confirmRestartWizard: function () {
      ...
      var loadingText = VortemWizard.formatLoadingText(
        vortemWizard.strings.loading || "Loading...",
      );
      ...
      $.ajax({
        ...
        success: function (response) {
          if (response.success) {
            ...
            window.location.href =
              ...
              vortemWizard.ajaxUrl.replace("admin-ajax.php", "admin.php") +
                "?page=vortem-setup-wizard&step=1";
          } else {
            alert(response.data.message || "Failed to restart wizard");
            $confirmButton
              .prop("disabled", false)
              .text(
                vortemWizard.strings && vortemWizard.strings.Restart
                  ? vortemWizard.strings.Restart
                  : "Restart",
              );
          }
        },
        error: function (xhr, status, error) {
          ...
          alert(
            "An error occurred while restarting the wizard. Please try again.",
          );
          $confirmButton
            .prop("disabled", false)
            .text(
    ...
    initTermsStep: function () {
      ...
      if (
        typeof vortemWizard === "undefined" ||
        !vortemWizard.hasSessionToken
      ) {
        VortemLogger.error(
          "Session token not found in terms step. Redirecting to features step.",
        );
        // Redirect to features step to complete authentication
        window.location.href =
          vortemWizard.ajaxUrl.replace("admin-ajax.php", "admin.php") +
          "?page=vortem-setup-wizard&step=3";
        return;
      }
      ...
      $.ajax({
        ...
        data: {
          ...
          terms_accepted:
            ...
            $("#accept-data-processing").is(":checked")
              ? "true"
              : "false",
          nonce: vortemWizard.nonce,
        },
        success: function (response) {
          ...
          if (response.success) {
            if (currentStep === 3 && typeof vortemWizard !== "undefined") {
              vortemWizard.termsAccepted = true;
            }
            VortemWizard.navigateToStep(response.data.redirect_url);
          } else {
      ...
      $.ajax({
        ...
        success: function (response) {
          if (response.success) {
            ...
            if ($buttonText.length) {
              $buttonText.text(
                vortemWizard.strings.back ||
                  vortemWizard.strings.previous ||
                  "Back",
              );
            } else {
              $button.text(
    ...
    completeWizard: function (e) {
      ...
      if (currentStep === 3) {
        ...
        if (typeof vortemWizard !== "undefined") {
          vortemWizard.termsAccepted = true;
        }

        // Navigate to step 4 (Complete) without a full page reload
        VortemLogger.log("Moving from step 3 to step 4");
        var currentUrl = new URL(window.location.href);
        currentUrl.searchParams.set("step", "4");
        VortemLogger.log("Navigating to:", currentUrl.toString());
        VortemWizard.navigateToStep(currentUrl.toString());
        return;
      }
      ...
      // Step 4: terms were validated on step 3; send true so server can persist if needed
      ...
      var currency =
        currencyValue && currencyValue.trim() !== ""
          ? currencyValue.trim()
          : typeof vortemWizard !== "undefined" && vortemWizard.currentCurrency
            ? vortemWizard.currentCurrency
            : "USD";

      VortemLogger.log(
        "VortemWizard: Completing setup with currency:",
        currency,
      );
    ...
    completeAndGoToDashboard: function (e) {
      ...
      if (
        ...
        VortemLogger.error("Session token not found. Cannot complete setup.");
        alert(
          vortemWizard.strings &&
            vortemWizard.strings["Please complete authentication first."]
            ? vortemWizard.strings["Please complete authentication first."]
      ...
      // Step 4 only: terms were required on step 3; always send accepted for server persistence
      var termsAccepted = true;

      // Disable link and show loading (<a> needs class hooks; [disabled] alone is unreliable)
      $button
        .prop("disabled", true)
      ...
      VortemLogger.log(
        "VortemWizard: Completing setup and going to dashboard with currency:",
        currency,
      );
          ...
          if (response.success) {
            ...
            alert(
              response.data.message ||
                (vortemWizard.strings &&
                vortemWizard.strings["Failed to complete setup"]
                  ? vortemWizard.strings["Failed to complete setup"]
      ...
      switch (step) {
        ...
        case 3: // Terms step
          ...
          if (
            ...
            VortemLogger.error(
              "Session token not found. Please complete authentication first.",
            );
            return false;
          }
          break;
    ...
    updateTermsStepButtonState: function () {
      ...
      if (bothAccepted) {
        ...
        $nextButton.prop("disabled", true).addClass("disabled");
        VortemLogger.log(
          "Next button disabled - both checkboxes must be checked",
        );
      }
    },
          ...
          $.ajax({
            ...
            success: function (response) {
              if (response.success) {
                ...
                VortemLogger.log(
                  "Terms and data processing consent accepted successfully",
                );
              }
            },
            error: function () {
      ...
      $.ajax({
        ...
        success: function (response) {
          ...
          if (response.success) {
            ...
            alert(
              vortemWizard.strings.error ||
                "An error occurred while switching currency",
            );
          }
        },
        error: function (xhr, status, error) {
  ...
  $(document).ready(function () {
    ...
    VortemLogger.log(
      "Restart wizard button found:",
      $("#restart-wizard").length,
    ); // Debug log
    VortemLogger.log(
      "Restart wizard modal found:",
      $("#restart-wizard-modal").length,
    ); // Debug log
  ...
  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", function () {
      VortemLogger.log(
        "DOMContentLoaded, pre-initializing currency display...",
      );
      if (
        typeof VortemWizard !== "undefined" &&
        VortemWizard.getCurrentStep() === 1
      ) {
